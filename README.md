    ...
    - [x] 싸이클 (순환의존성)
        - [x] 예외 발생
            ...
            - 어느 빈이 해당 문제를 발생 시키는지
                - [ ] 어떤 관계가 싸이클을 만드는지도 알려줬으면 좋겠다 (어떤 빈을 참조하려고 하니깐 싸이클이 되는건지)
    - 생성에 필요한 정보를 기록하는 부분이 필요한 것 같다..
        - 특정 어노테이션 스캔 이외에도 Configuration 등 여러가지가 동시에 등록되는 경우가 많다...
            - 특정 인스턴스가 생성될 때 필요한 정보(ex. di 되어야 하는 애들) 는 어떻게 알 수 있을까?
                ...
                - 정보를 어떤 형태로 포함할까?
    - [x] `@Inject` 로 설정된 빈이 존재하지 않는 경우
        - 이 경우 어떤 곳에서 어떤 빈이 존재 하지 않는지 알려줄 수 있는 방안이 필요함
    - [x] `@Inject` 로 설정된 곳에 가능한 빈이 여러개 존재할 경우
            - [x] 예외처리
            - `@Qualifier` 구현하기
    - [x] 특정 인터페이스를 구현한 빈이 존재하지 않을 경우
        - `@Inject` 를 통해서 생성할 때 발생할 수 있음
